...
> 当前阶段：技术基线已冻结，WP-00 至 WP-08 已完成
...
- 更正文件必须使用新批次并引用原批次；
- 不覆盖原导入记录；
- 同一完整文件内撤销行先于原核销行时，导入完成后统一匹配，行顺序不影响结果；
- WP-10 复验确认当前版本没有跨文件自动重开旧批次的匹配窗口；跨批迟到事实保留原差异，并通过引用原批次的新更正批次处理；如真实厂商要求自动回溯，必须新增旧批次不可改写和已确认结算调整设计；
...
该方案能够将现有点评秒杀工程能力迁移为连锁品牌营销权益的技术底座，模拟技术评审结论为“修改后通过”。当前 WP-00 至 WP-09 已完成：契约与领域骨架、身份与门店、活动与库存、领取受理/Outbox、异步发券、会员卡包、POS 履约、当天撤销、T+1 对账、结算复核、运营查询和基础观测均已有代码与自动化验证；系统级发布演练和成果整理仍待后续工作包实现。
...
- 不将模拟契约当成真实厂商文档；
- 不将旧链路压测数字作为新平台成果；
- 不把本地模拟的 POS、对账与结算能力描述为真实厂商联调、真实付款或生产化结论。

当前开发状态为 `WP-00 至 WP-09 DONE`，M2—M4 已完成本地模拟门禁，M5 已进入进行中。下一工作包为 WP-10 系统验证与发布演练；在 WP-10—WP-11 Gate 通过前，不把当前能力描述成已经完成生产化验收或上线。
