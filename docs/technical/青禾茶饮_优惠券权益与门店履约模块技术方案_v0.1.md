...
sequenceDiagram
    ...
    O->>D: 标记事件SENT
    M->>C: 投递发放命令
    C->>D: 事务创建Entitlement、累计issued、Claim→SUCCESS
    C-->>M: ACK
...
- Publisher ACK 后只允许标记 `SENT`，不得更新领取申请为成功；
- `SENDING` 超过租约时间可重新抢占；
...
| 编号 | 问题 | 责任人 | 截止时间 | 关闭证据 |
|---|---|---|---|---|
| DEC-A01 | 新领取链路同步写 ClaimRequest + Outbox 的峰值能力 | 后端开发 | WP-04 完成前 | 新链路压测报告及环境、数据规模、P95/P99、错误率 |
...
该方案能够将现有点评秒杀工程能力迁移为连锁品牌营销权益的技术底座，模拟技术评审结论为“修改后通过”，意见已经回写并完成开发工作包拆分。WP-00 Mock 与契约基础设施已经完成；正式领域业务仍处于待实现阶段。
...
本轮只完成了无代码的设计、评审回写和任务规划；开发状态为 `NOT_STARTED`。下一次进入集中编码时，从 Mock/契约测试骨架和 P0 用例开始。
