...
本文描述“双方怎么说话”，不表示业务功能已经实现。接口可被 Mock 调用，也不表示真实系统联调通过。
...
WP-00 已完成，证据见 [WP-00 契约基础设施验证记录](./WP00_契约基础设施验证记录_v1.0.md)。下一步进入 WP-01 建立新领域工程与数据迁移骨架；业务实现完成前，行为断言继续保持未运行或明确跳过，不伪造完整链路结果。
