...
本文描述“双方怎么说话”。截至 2026-09-10，POS 验券、核销、原请求查询和当天撤销已完成本地模拟业务实现；对账等仍按后续工作包推进。接口可被 Mock 调用或在本地实现中通过测试，均不表示真实系统联调或生产上线。
...
WP-00 契约基础设施已完成，证据见 [WP-00 契约基础设施验证记录](./WP00_契约基础设施验证记录_v1.0.md)。截至 WP-07，POS-01 至 POS-04 均已有本地实现和可执行行为测试。任何本地通过结果都不冒充真实厂商联调或生产证据。
