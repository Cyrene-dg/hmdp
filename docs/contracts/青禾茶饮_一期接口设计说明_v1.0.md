...
本文描述“双方怎么说话”。截至 2026-09-10，POS 验券、核销和原请求查询已完成本地模拟业务实现；撤销、对账等仍按后续工作包推进。接口可被 Mock 调用或在本地实现中通过测试，均不表示真实系统联调或生产上线。
...
至少支持：正常文件、重复文件、同批次 checksum 冲突、合法空文件、文件缺失、坏行、行数不一致、更正批次、撤销先到。
...
WP-00 契约基础设施已完成，证据见 [WP-00 契约基础设施验证记录](./WP00_契约基础设施验证记录_v1.0.md)。截至 WP-06，POS-01、POS-02、POS-03 已有本地实现和可执行行为测试；POS-04 撤销仍由 WP-07 实现。任何本地通过结果都不冒充真实厂商联调或生产证据。
